...
[else]
	...
	;亡国
	[eval exp="kag.se[7].volume2 = sf.dummy_seopt6"]
	[if exp="f.bgvnow == 1"]
		;モブのvoiceをオンにしたよフラグ。BGVの再生し直しに使う。
		[eval exp="f.mob_voice_on = 1"]
	[endif]
	;スライダーを再描画しにいく
	[call storage="system_config_srd.ks" target=*chvolume02]
...
[else]
	...
	[endif]
	;ボイス音量を元に戻す
	[eval exp="kag.se[16].volume2 = sf.dummy_seopt16"]
...
;	[call storage="system_config_srd.ks" target=*chvolume6]
;	;つじつま合わせ
;	[if exp="sf.dummy_seopt16 <=100"]
;		[eval exp="sf.dummy_seopt16 = sf.dummy_seopt16*1000"]
;	[endif]
...
[else]
	...
	[endif]
	;ボイス音量を元に戻す
	[eval exp="kag.se[14].volume2 = sf.dummy_seopt14"]
