...
;			;システムボタン＆ウィンドウ消去
;			[sysbt_meswin clear]
;	;窓のオンオフ制御に使うんだっけ？面倒だからカットインではオンにしないでおく
;	;		[eval exp="f.evcg_on = 1"]
;		[endif]
...
;	[image storage="chara_int" layer="&sf.toplayer" page=back visible=true left=0 top=0]
	...
	@if exp="(existentImage(mp.storage)!==false)"
		...
		;* を書くと、マクロの中で指定された属性がいちいち書かなくても全部%になるの。でも書かなくても0指定したいから書く
		;imaegeに書くと長くて読みにくいからレイヤ設定にデフォを記述
		[layopt layer=%layer|1 page=back opacity=%opacity|255 grayscale=%grayscale|false rgamma=%rgamma|1.0 ggamma=%ggamma|1.0 mcolor=%mcolor|false mopacity=%mopacity|false rfloor=%rfloor|0 gfloor=%gfloor|0 bfloor=%bfloor|0 rceil=%rceil|255 gceil=%gceil|255 bceil=%bceil|255 fliplr=%fliplr|false flipud=%flipud|false]
		[image storage=%storage layer=%layer|1 page=back visible=true  left=%x|0 top=%y|0 grayscale=%grayscale|false mcolor=%mcolor|false mopacity=%mopacity|false]
		[eval exp="sf[mp.storage]= 1"]
		[eval exp="f[mp.storage] = 1"]
;マスク呼び出すフラグはオフのまま
;		[eval exp="f.mskon = 1"]
...
[macro name=cutin_int]
	;特にマクロ化の意味はないが検索用に作成
	[chara_int_ layer=%layer|1]
[endmacro]
